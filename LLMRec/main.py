    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            if val_ret['recall'][1] > best_recall:
                ...
                best_metrics = copy.deepcopy(test_ret)
                best_epoch = epoch
                # Optionally dump propagated embeddings at the best-val epoch
                if args.save_item_embedding_path or args.save_user_embedding_path:
                    self.model_mm.eval()
                    with torch.no_grad():
                        ua_emb_best, ia_emb_best, *_ = self.model_mm(
                    ...
                    if args.save_user_embedding_path:
                        os.makedirs(os.path.dirname(args.save_user_embedding_path) or '.', exist_ok=True)
                        np.save(args.save_user_embedding_path, ua_emb_best.detach().cpu().numpy())
                        self.logger.logging("Saved user embeddings -> %s" % args.save_user_embedding_path)
