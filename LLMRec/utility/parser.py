...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='?',
        default='shared',
        choices=['separate', 'shared', 'shared_adapter'],
        help='Projection sharing for text, user profile, and item attribute features.'
    )
    parser.add_argument(
        '--text_adapter_hidden_dim',
        type=int,
        default=16,
        help='Hidden size of domain adapters when --text_encoder_mode=shared_adapter.'
    )
    parser.add_argument('--use_image_feat', type=int, default=1, choices=[0, 1], help='Whether to use image features in the multimodal branch.')
    parser.add_argument('--use_text_feat', type=int, default=1, choices=[0, 1], help='Whether to use text features in the multimodal branch.')
    parser.add_argument('--use_item_attribute', type=int, default=1, choices=[0, 1], help='Whether to use item attribute augmentation features.')
    parser.add_argument('--use_user_profile', type=int, default=1, choices=[0, 1], help='Whether to use user profile augmentation features.')
    parser.add_argument('--use_sample_augmentation', type=int, default=1, choices=[0, 1], help='Whether to use augmented training samples.')
    parser.add_argument(
        '--user_profile_variant',
        default='pooled',
        choices=['pooled', 'history_summary', 'structured_profile'],
        help='Which prepared user profile embedding variant to load.'
    )
    parser.add_argument(
        '--user_profile_path',
        default='',
        type=str,
        help='Optional explicit path to a prepared user profile embedding pickle.'
    )
    ...
    parser.add_argument('--save_user_embedding_path', default='', type=str, help='Optional .npy path to dump the propagated user embeddings at the best-val epoch.')
